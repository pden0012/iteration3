...
console.log(`🔗 Backend URL: http://15.134.34.242:8080`);
...
if (fs.existsSync(staticPath)) {
  ...
  // Serve static files with /iteration3 prefix
  app.use('/iteration3', express.static(staticPath));
  // Also serve without prefix for fallback
  ...
  onError: (err, req, res) => {
    ...
    if (err.code === 'ECONNREFUSED') {
      ...
      res.status(502).json({ 
        ...
        code: 'ECONNREFUSED',
        details: 'The proxy server cannot establish a connection to the backend API server at http://15.134.34.242:8080',
        timestamp: new Date().toISOString(),
        retryable: true
      });
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    timestamp: new Date().toISOString(),
    proxy: 'Active',
    backend: 'http://15.134.34.242:8080',
    port: PORT,
    nodeEnv: process.env.NODE_ENV,
...
app.get('/api/test-backend', async (req, res) => {
  try {
    const response = await fetch('http://15.134.34.242:8080/dashboard?suburb=Melbourne&format=json', {
      timeout: 5000
    });
    
    res.json({
      status: 'Backend reachable',
      backendUrl: 'http://15.134.34.242:8080',
...
// Catch all handler - serve index.html for Vue Router (but not for static assets)
app.get('*', (req, res) => {
  // Skip static assets - they should be handled by express.static
  if (req.url.startsWith('/iteration3/assets/') || req.url.startsWith('/assets/')) {
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Proxying API requests to: http://15.134.34.242:8080`);
